...
describe('App', () => {
      ...
      if (url.endsWith('/auth/demo-login')) {
        ...
        return {
          ...
          json: async () => ({
            ...
            displayName,
            role,
            walletAddress: role === 'CUSTOMER' ? '0x1111111111111111111111111111111111111111' : undefined,
  ...
  it('submits asset form and calls the API', async () => {
    ...
    await waitFor(() => {
      const assetCall = mockFetch.mock.calls.find((call) => call[0].endsWith('/assets'));
  ...
  it('lists returned asset for sale and calls the marketplace/listings API', async () => {
    ...
    await waitFor(() => {
      ...
      expect(body.sellerId).toBe('customer-1');
    ...
    await waitFor(() => {
      const listingCall = mockFetch.mock.calls.find((call) => {
        ...
        try {
          ...
          return call[0].endsWith('/marketplace/listings') && body.sellerId === 'customer-999';
        } catch {
          return false;
        }
      });
